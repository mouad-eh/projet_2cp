...
from auto_ui.ui_monnaie2 import Ui_Form as Monnaie_form
from auto_ui.ui_ajout_monnaie import Ui_Form as Ajout_monnaie_form
from auto_ui.ui_modifier_monnaie import Ui_Form as Modifier_monnaie_form
...
class MonnaieWindow(QWidget):
    def __init__(self):
        ...
        self.ui.code.textChanged.connect(self.rechercher_monnaie_code)
        self.ui.libelle.textChanged.connect(self.rechercher_monnaie_libelle)
        ...
        self.ui.valider_monnaie.clicked.connect(self.valider_monnaie)
    ...
    def rechercher_monnaie_code(self):
        sql = "SELECT * FROM monnaie WHERE code_monnaie LIKE '{}%'".format(self.ui.code.text())
        self.display_table(sql)

    def rechercher_monnaie_libelle(self):
        sql = "SELECT * FROM monnaie WHERE libelle_monnaie LIKE '{}%'".format(self.ui.libelle.text())
        self.display_table(sql)
    ...
    def valider_monnaie(self):
        selectedItems = self.ui.monnaie_table.selectedItems()
        print(self.ui.monnaie_table.item(self.ui.monnaie_table.currentRow().numerator,0).text())

        if len(selectedItems) == 3:
         self.rowmonnaie = (self.ui.monnaie_table.verticalHeaderItem(selectedItems[0].row()).text(),self.ui.monnaie_table.item(self.ui.monnaie_table.currentRow().numerator,0).text(),self.ui.monnaie_table.item(self.ui.monnaie_table.currentRow().numerator,2).text())
        print(self.rowmonnaie)
        self.close()
    ...
    def ajouter_ajout_monnaie(self):
        ...
        monnaie = Monnaie(0)
        monnaie_id = monnaie.inserer(code_monnaie, libelle_monnaie, taux_change)
